//моя первая игра, версия от 15.11.21
...
function moove_player_ship () {
	let temp = 0; //менная для фильтрации первого опроса и предотвращения повторных срабатываний	
	document.addEventListener('keydown', function(event) {
		let player = document.getElementById('player');
		if (event.key ==  'ArrowRight' && temp == 0) {		
...
setInterval(function () {
	if (game_over == 0) {
		moove_player_ship (); //запуск контроля перемещения корабля
	}
}, 400);
setInterval(function () {
	if (game_over == 0) {
